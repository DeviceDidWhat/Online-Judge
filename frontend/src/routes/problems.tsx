...
function Problems() {
  ...
  const stats = [
    ...
    { label: "Easy", value: problems.filter((p) => p.difficulty === "Easy" && p.solved).length, total: problems.filter((p) => p.difficulty === "Easy").length, color: "text-success" },
    { label: "Medium", value: problems.filter((p) => p.difficulty === "Medium" && p.solved).length, total: problems.filter((p) => p.difficulty === "Medium").length, color: "text-warning" },
    { label: "Hard", value: problems.filter((p) => p.difficulty === "Hard" && p.solved).length, total: problems.filter((p) => p.difficulty === "Hard").length, color: "text-destructive" },
  ];
      ...
      <div className="mx-auto max-w-7xl p-6">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{problems.length} problems · curated by editors</p>
          </div>
